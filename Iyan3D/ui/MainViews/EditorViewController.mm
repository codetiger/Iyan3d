...
- (void)createDuplicateAssetsForAnimation {
    ...
    if ((selectedNodeType == NODE_RIG || selectedNodeType == NODE_TEXT_SKIN || selectedNodeType == NODE_TEXT || selectedNodeType == NODE_SGM || selectedNodeType == NODE_OBJ)) {
        ...
        [self load3DTex:(selectedNodeType == NODE_TEXT) ? ASSET_TEXT : ASSET_TEXT_RIG
                AssetId:0
            TextureName:[NSString stringWithCString:editorScene->nodes[selectedNode]->getProperty(TEXTURE).fileName.c_str()
                                           encoding:[NSString defaultCStringEncoding]]
              TypedText:typedText
               FontSize:fontSize
             BevelValue:bevalValue
              TextColor:color
               FontPath:fontName
             isTempNode:NO];
    }
...
- (void)changeTextureForAsset:(PROP_INDEX)pIndex {
    ...
    if ([Utility IsPadDevice]) {
        ...
        [self.popoverController dismissPopoverAnimated:YES];
        objVc                  = [[ObjSidePanel alloc] initWithNibName:@"ObjSidePanelPhone" bundle:Nil Type:CHANGE_TEXTURE AndPropIndex:pIndex];
        objVc.objSlideDelegate = self;
        [self showOrHideLeftView:YES withView:objVc.view];
    }
    NSInteger toolBarPos                        = [[[AppHelper getAppHelper] userDefaultsForKey:@"toolbarPosition"] integerValue];
...
- (void)importBtnDelegateAction:(int)indexValue //TODO remove downloading json and showing store models
{
    switch (indexValue) {
        ...
        case IMPORT_OBJFILE: {
            if ([Utility IsPadDevice]) {
                [self.popoverController dismissPopoverAnimated:YES];
                objVc                  = [[ObjSidePanel alloc] initWithNibName:@"ObjSidePanel" bundle:Nil Type:IMPORT_OBJFILE AndPropIndex:UNDEFINED];
                objVc.objSlideDelegate = self;
                [self showOrHideLeftView:YES withView:objVc.view];
            } else {
                [self.popoverController dismissPopoverAnimated:YES];
                objVc                  = [[ObjSidePanel alloc] initWithNibName:@"ObjSidePanelPhone" bundle:Nil Type:IMPORT_OBJFILE AndPropIndex:UNDEFINED];
                objVc.objSlideDelegate = self;
                [self showOrHideLeftView:YES withView:objVc.view];
            }
            NSInteger toolBarPos                        = [[[AppHelper getAppHelper] userDefaultsForKey:@"toolbarPosition"] integerValue];
...
- (void)exportBtnDelegateAction:(int)indexValue {
    ...
    if (indexValue == EXPORT_IMAGE) {
        ...
        if ([Utility IsPadDevice]) {
            ...
            renderingView             = [[RenderingViewController alloc] initWithNibName:([self iPhone6Plus]) ? @"RenderingViewControllerPhone@2x" : @"RenderingViewControllerPhone" bundle:nil StartFrame:editorScene->currentFrame EndFrame:editorScene->totalFrames renderOutput:RENDER_IMAGE caMresolution:cameraResolutionType ScreenWidth:ScreenWidth ScreenHeight:ScreenHeight];
            renderingView.delegate    = self;
            BOOL status               = ([[[AppHelper getAppHelper] userDefaultsForKey:@"toolbarPosition"] integerValue] == TOOLBAR_LEFT);
            renderingView.projectName = currentScene.name;
            renderingView.sgbPath     = [self getSGBPath];
        ...
        if ([Utility IsPadDevice])
            ...
            renderingView = [[RenderingViewController alloc] initWithNibName:(ScreenHeight > 320) ? @"RenderingViewControllerPhone@2x" : @"RenderingViewControllerPhone" bundle:nil StartFrame:0 EndFrame:editorScene->totalFrames renderOutput:RENDER_VIDEO caMresolution:cameraResolutionType ScreenWidth:ScreenWidth ScreenHeight:ScreenHeight];
        renderingView.delegate    = self;
        renderingView.projectName = currentScene.name;
        renderingView.sgbPath     = [self getSGBPath];
        if ([Utility IsPadDevice] || ScreenHeight < 400)
...
- (void)changedPropertyAtIndex:(PROP_INDEX)index WithValue:(Vector4)value AndStatus:(BOOL)status {
    ...
    switch (index) {
        ...
        case BUMP_MAP:
        case TEXTURE: {
            [self changeTextureForAsset:index];
            break;
        }
        case TEXTURE_SCALE: {
            editorScene->actionMan->storePropertyChangeAction(editorScene->selectedNodeId, value, index, status);
