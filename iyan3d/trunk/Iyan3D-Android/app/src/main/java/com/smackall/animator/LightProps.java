...
public class LightProps implements View.OnTouchListener, SeekBar.OnSeekBarChangeListener, View.OnClickListener, RadioButton.OnCheckedChangeListener {
    ...
    public void showLightProps(View v, MotionEvent event) {
        ...
        switch (UIHelper.ScreenType) {
            case Constants.SCREEN_NORMAL:
                light_prop.getWindow().setLayout(Constants.width / 2, Constants.height);
                break;
            default:
        ...
        if (nodeType == Constants.NODE_LIGHT) {
            dialog.findViewById(R.id.distanceOptionHolder).setVisibility(View.GONE);
        }
        light_prop.show();
    ...
    private void initViews(Dialog light_prop) {
        light_prop.findViewById(R.id.color_picker).setOnTouchListener(this);
        ...
        light_prop.findViewById(R.id.delete_animation).setOnClickListener(this);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        touchedRGB = ((EditorView) mContext).colorPicker.getColorFromPixel(v, event, bitmap);
