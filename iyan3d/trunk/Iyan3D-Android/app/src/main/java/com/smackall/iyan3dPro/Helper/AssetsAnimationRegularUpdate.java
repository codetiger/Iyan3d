...
public class AssetsAnimationRegularUpdate {

    private String url;
    private Context context;
    private SharedPreferenceManager sp;

    public AssetsAnimationRegularUpdate(Context mContext, SharedPreferenceManager sp) {
        this.context = mContext;
        this.sp = sp;
    }

    public void jsonParse(String url, int jsonType) {
        this.url = url;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            new JSONParse(context, sp, jsonType).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        } else {
            new JSONParse(context, sp, jsonType).execute();
        }
    }
    ...
    private class JSONParse extends AsyncTask<Integer, Integer, String> {
        ...
        public JSONParse(Context context, SharedPreferenceManager sp, int jsonType) {
