...
public class HitScreens {
    ...
    public static void NewsFeed(Context mContext) {
        AnalyticsApplication application = (AnalyticsApplication) ((Activity) mContext).getApplication();
        Tracker mTracker = application.getDefaultTracker();
        mTracker.setScreenName("News Feed" + " - Android");
        mTracker.send(new HitBuilders.ScreenViewBuilder().build());
    }

    public static void PropsView(Context mContext, int nodeType) {
        String nodeName = (nodeType == Constants.NODE_CAMERA) ? "Camera" : (nodeType == Constants.NODE_LIGHT || nodeType == Constants.NODE_ADDITIONAL_LIGHT)
                ? "Light" : (nodeType == Constants.NODE_SGM || nodeType == Constants.NODE_RIG ||
                nodeType == Constants.NODE_OBJ || nodeType == Constants.NODE_TEXT || nodeType == Constants.NODE_TEXT_SKIN) ? "Mesh" : (nodeType == Constants.NODE_IMAGE) ? "Image" :
                (nodeType == Constants.NODE_PARTICLES) ? "Particle" : "Scene";

        AnalyticsApplication application = (AnalyticsApplication) ((Activity) mContext).getApplication();
        Tracker mTracker = application.getDefaultTracker();
        mTracker.setScreenName(nodeName + " Props" + " - Android");
        mTracker.send(new HitBuilders.ScreenViewBuilder().build());
    }
}
