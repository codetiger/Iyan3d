...
public class CameraProps implements View.OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    public void showCameraProps(View v, MotionEvent event) {
        ...
        switch (UIHelper.ScreenType) {
            case Constants.SCREEN_NORMAL:
                ...
                break;
            default:
                camera_prop.getWindow().setLayout(Constants.width / 2, Constants.height / 2);
                break;
        }
    ...
    private void initViews(Dialog camera_prop) {
        camera_prop.findViewById(R.id.THOUSAND_EIGHTY).setOnClickListener(this);
        camera_prop.findViewById(R.id.SEVEN_TWENTY).setOnClickListener(this);
        camera_prop.findViewById(R.id.FOUR_EIGHTY).setOnClickListener(this);
        camera_prop.findViewById(R.id.THREE_SIXTY).setOnClickListener(this);
        camera_prop.findViewById(R.id.TWO_FOURTY).setOnClickListener(this);
        camera_prop.findViewById(R.id.delete).setOnClickListener(this);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.delete) {
            ((EditorView) mContext).delete.showDelete();
    ...
    private void update(boolean storeAction) {
        ((EditorView) mContext).renderManager.cameraProperty(((SeekBar) dialog.findViewById(R.id.field_of_view)).getProgress(), sp.getInt(mContext, "cameraResolution"), storeAction);
    }
}
