...
public class Export implements View.OnClickListener, CreditTask, CompoundButton.OnCheckedChangeListener {
    ...
    private void initViews(Dialog export) {
        ...
        export.findViewById(R.id.normal).setOnClickListener(this);
        export.findViewById(R.id.toon).setOnClickListener(this);
        export.findViewById(R.id.cloud).setOnClickListener(this);
        ...
        helpHint.setText(String.format(Locale.getDefault(), "%s", mContext.getResources().getString(R.string.renderNormalMsg)));
        highLight(export.findViewById(R.id.normal));
    ...
    private void highLight(View v) {
        ((ViewGroup) dialog.findViewById(R.id.normal).getParent()).setBackgroundColor((v.getId() == R.id.normal) ? Color.WHITE : 0x00000000);
        ((ViewGroup) dialog.findViewById(R.id.toon).getParent()).setBackgroundColor((v.getId() == R.id.toon) ? Color.WHITE : 0x00000000);
        ((ViewGroup) dialog.findViewById(R.id.cloud).getParent()).setBackgroundColor((v.getId() == R.id.cloud) ? Color.WHITE : 0x00000000);
    }

    private void disableOptions(int renderType) {
        try {
            waterMark = (renderType != Constants.CLOUD);
            dialog.findViewById(R.id.waterMark).setAlpha((renderType != Constants.CLOUD) ? 1.0f : 0.3f);
            ((Switch) dialog.findViewById(R.id.waterMark)).setChecked((renderType != Constants.CLOUD));
            dialog.findViewById(R.id.waterMark).setClickable((renderType != Constants.CLOUD));
            dialog.findViewById(R.id.color_picket_btn).setAlpha((renderType != Constants.CLOUD) ? 1.0f : 0.3f);
            dialog.findViewById(R.id.color_picket_btn).setClickable((renderType != Constants.CLOUD));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.toon:
                ...
                helpHint.setText(String.format(Locale.getDefault(), "%s", mContext.getResources().getString(R.string.renderToonMsg)));
            ...
            case R.id.cloud:
                ...
                helpHint.setText(String.format(Locale.getDefault(), "%s", mContext.getResources().getString(R.string.renderCloudMsg)));
            ...
            case R.id.next:
                if (((EditorView) mContext).userDetails.signInType <= 0) {
                    informDialog(mContext, mContext.getResources().getString(R.string.pleseSignin));
                ...
                dialog.findViewById(R.id.creditProgress).setVisibility(View.VISIBLE);
                ((EditorView) mContext).creditsManager.getCreditsForUniqueId(Export.this);
                break;
            case R.id.done:
                HitScreens.EditorView(mContext);
                cancel = true;
    ...
    private void nextBtnAction(int currentCredits, int premiumUser) {
        ...
        if (updateCreditLable() > currentCredits && premiumUser <= 0) {
            informDialog(mContext, mContext.getResources().getString(R.string.notenoughCredit));
            dialog.findViewById(R.id.next).setVisibility(View.VISIBLE);
            ...
            if (!canUploadToCloud()) {
                dialog.findViewById(R.id.next).setVisibility(View.VISIBLE);
                dialog.findViewById(R.id.creditProgress).setVisibility(View.GONE);
                UIHelper.informDialog(mContext, mContext.getResources().getString(R.string.sceneContainParticle));
                enableControls(dialog, true);
                return;
            }
            ((EditorView) mContext).glView.queueEvent(new Runnable() {
                ...
                public void run() {
                    ...
                    GL2JNILib.save(((EditorView) mContext).nativeCallBacks, true, ((EditorView) mContext).projectNameHash, false);
                }
            });
        }
    }
    ...
    public void packCloudRender() {
        if (((EditorView) mContext).zipManager.packForCloudRender()) {
            Events.exportNextAction(mContext);
            ((EditorView) mContext).publish.uploadHQRender(min, max, (credit * -1), getCameraResolution()[0], getCameraResolution()[1]);
        } else {
            if (dialog != null && dialog.isShowing())
                dialog.dismiss();
            UIHelper.informDialog(mContext, mContext.getResources().getString(R.string.somthingwrongmsg));
        }
    }
    ...
    private int updateCreditLable() {
        if (Constants.isPremium(mContext) && (shader != Constants.CLOUD))
            dialog.findViewById(R.id.credit_lable).setVisibility(View.INVISIBLE);
        else {
            ...
            ((TextView) dialog.findViewById(R.id.credit_lable)).setText((credits == 0) ? "" : String.format(Locale.getDefault(), "%d %s", credits, mContext.getResources().getString(R.string.Credits)));
            ...
            public void run() {
                ...
                if (type == Constants.EXPORT_IMAGES) {
                    if (usageType.equals("240P")) finishExport(frame, true, "");
                        ...
                        ((EditorView) mContext).creditsManager.useOrRechargeCredits(credit * -1, usageType, "", "", frame, Export.this);
                }
            }
        });
    }
                    ...
                    try {
                        if (frameCount >= max && !cancel) {
                            if (usageType.equals("240P")) finishExport(0, true, "");
                                ...
                                ((EditorView) mContext).creditsManager.useOrRechargeCredits(credit * -1, usageType, "", "", 0, Export.this);
                            cancel = true;
                            return;
                        }
    ...
    private void enableControls(Dialog export, boolean state) {
        ...
        export.findViewById(R.id.normal).setEnabled(state);
        export.findViewById(R.id.toon).setEnabled(state);
        export.findViewById(R.id.cloud).setEnabled(state);
        export.findViewById(R.id.waterMark).setEnabled(state);
        ...
        export.findViewById(R.id.normal).setAlpha((state) ? 1.0f : 0.5f);
        export.findViewById(R.id.toon).setAlpha((state) ? 1.0f : 0.5f);
        export.findViewById(R.id.cloud).setAlpha((state) ? 1.0f : 0.5f);
        export.findViewById(R.id.waterMark).setAlpha((state) ? 1.0f : 0.5f);
