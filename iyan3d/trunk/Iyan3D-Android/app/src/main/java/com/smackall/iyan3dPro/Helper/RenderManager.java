...
import com.smackall.iyan3dPro.Rig;
...
public class RenderManager {
    ...
    public void importAssets(final AssetsDB assetsDB) {
        glView.queueEvent(new Runnable() {
            ...
            public void run() {
                ...
                GL2JNILib.importModel(((EditorView) mContext).nativeCallBacks,assetsDB.getAssetName(),
    ...
    public void changeTexture(final int selectedNodeId, final int selectedMeshBufferid, final String textureName, final float x, final float y, final float z, final boolean isTemp, final int index) {
        glView.queueEvent(new Runnable() {
            @Override
            public void run() {
                ((EditorView) mContext).showOrHideLoading(Constants.SHOW);
                GL2JNILib.changeTexture(selectedNodeId, selectedMeshBufferid, textureName, x, y, z, isTemp, index);
                ((EditorView) mContext).showOrHideLoading(Constants.HIDE);
            }
        });
    }
