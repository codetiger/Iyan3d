...
public class Constants {
    ...
    public static boolean isPremium(Context mContext) {
        ...
        try {
            if (!key.exists() || KeyMaker.readFromKeyFile(mContext) == null) return false;
            else try {
                return KeyMaker.readFromKeyFile(mContext).equals(FileHelper.md5(deviceUniqueId + FileHelper.md5("Iyan3dPremium" + 1)));
            } catch (IOException e) {
                e.printStackTrace();
            }
        } catch (IOException e) {
