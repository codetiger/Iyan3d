...
bool SGPreviewScene::applyAnimations(int assetId)
{
    ...
    #endif
    #ifdef ANDROID
    filePath = constants::DocumentsStoragePath + "/animations/" + to_string(assetId) + extension;
    #endif
...
{
    Logger::log(INFO, "SGPreviewScene", "Stop Animation");
    Logger::log(INFO, "SGPreviewScene", "Total Frames : " + to_string(totalFrames));
    Logger::log(INFO, "SGPreviewScene", "Current Frames : " + to_string(currentFrame));
