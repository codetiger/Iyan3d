...
bool SGSceneLoader::loadSceneData(JNIEnv *env, jclass type,jobject object,std::string *filePath)
{
    if(!currentScene || !smgr)
        ...
        ifstream inputSGBFile(*filePath,ios::in | ios::binary );
        FileHelper::resetSeekPosition();
        if(!readScene(env,type,object,&inputSGBFile)){
            inputSGBFile.close();
            return false;
        }
        inputSGBFile.close();
        ...
        if(sgbVersion == SGB_VERSION_CURRENT) {
            ...
            for(int i = 0;i < nodeCount;i++) {
                ...
                if(origId > 0) {
                    ...
                    status = currentScene->downloadMissingAssetsCallBack(env,type, object, to_string(sgNode->assetId),sgNode->getType(), !(sgNode->getProperty(IS_VERTEX_COLOR).value.x || sgNode->getProperty(TEXTURE).fileName == "" || sgNode->getProperty(TEXTURE).fileName == "-1"), sgNode->getProperty(TEXTURE).fileName);
                } else if (sgNode->getType() == NODE_TEXT_SKIN || sgNode->getType() == NODE_TEXT) {
                    status = currentScene->downloadMissingAssetsCallBack(env,type,object,sgNode->optionalFilePath,sgNode->getType(), !(sgNode->getProperty(IS_VERTEX_COLOR).value.x || sgNode->getProperty(TEXTURE).fileName == "" || sgNode->getProperty(TEXTURE).fileName == "-1"), sgNode->getProperty(TEXTURE).fileName);
                } else if (sgNode->getType() == NODE_IMAGE) {
                    status = currentScene->downloadMissingAssetsCallBack(env,type,object,ConversionHelper::getStringForWString(sgNode->name), sgNode->getType(), !(sgNode->getProperty(IS_VERTEX_COLOR).value.x), sgNode->getProperty(TEXTURE).fileName);
                } else if (sgNode->getType() == NODE_PARTICLES) {
                    status = currentScene->downloadMissingAssetsCallBack(env,type,object,to_string(sgNode->assetId), sgNode->getType(), true, sgNode->getProperty(TEXTURE).fileName);
                }
