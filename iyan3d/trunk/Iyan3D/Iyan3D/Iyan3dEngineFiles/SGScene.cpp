...
bool SGScene::calculateControlMovements(Vector2 curPoint,Vector2 prevTouchPoint,Vector3 &outputValue,bool isSGJoint)
{
    ...
    if(controlType == MOVE){ // position and scale calculations are same
        outputValue.x = ((selectedControlId == X_MOVE) ? 1.0 : 0.0) * (newPos.x - oldPos.x);
        outputValue.y = ((selectedControlId == Y_MOVE) ? 1.0 : 0.0) * (newPos.y - oldPos.y);
        outputValue.z = ((selectedControlId == Z_MOVE) ? 1.0 : 0.0) * (newPos.z - oldPos.z);
        return true;
    }
