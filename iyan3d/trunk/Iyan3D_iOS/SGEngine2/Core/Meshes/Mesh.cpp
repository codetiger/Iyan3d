...
void Mesh::Commit() {
    ...
    if(meshType == MESH_TYPE_LITE) {
        ...
        if((meshBufferIndices[meshBufferIndex].size() + indicesDataCopy.size()) > MAX_VERTICES_COUNT) {
        ...
        for(int i = 0; i < (int)indicesDataCopy.size(); i++) {
            if((int)meshBufferIndices[meshBufferIndex].size() == MAX_VERTICES_COUNT) {
                meshBufferVerticesData.push_back(vector< vertexData >());
                meshBufferIndices.push_back(vector< unsigned short >());
                meshBufferIndex++;
            }
        ...
        for (int meshBufferIndex = 0; meshBufferIndex < meshBufferCount; meshBufferIndex++) {
            ...
            int vertexCount = (tempVerticesDataHeavy.size() >= MAX_VERTICES_COUNT) ? MAX_VERTICES_COUNT : (int)tempVerticesDataHeavy.size();
