...
#include <math.h>
#include <algorithm>
#include "Font/glu/include/glu.h"

#ifdef _ANDROID__

#include "Font/iGLU/include/glu.h"
#endif
...
void addVertexDataFromTri(Tri t, Mesh* mesh)
{
    ...
    mesh->addToHighPolyIndicesArray(mesh->getVerticesCount() - 1);
...
float Font2OBJ::AddCharacterSideFace(FT_Face face, char ch, unsigned short bezierSteps, float offset, float extrude, SkinMesh* mesh, float bevelRadius, int bevelSegments)
{
...
float Font2OBJ::AddCharacter(FT_Face face, char ch, unsigned short bezierSteps, float offset, float extrude, SkinMesh* mesh, float bevelRadius, int bevelSegments)
{
    static FT_UInt prevCharIndex = 0, curCharIndex = 0;
    static FT_Pos prev_rsb_delta = 0;
...
float Font2OBJ::AddBevel(FT_Face face, char ch, unsigned short bezierSteps, float offset, float extrude, SkinMesh* mesh, float bevelRadius, int bevelSegments)
{
    static FT_UInt prevCharIndex = 0, curCharIndex = 0;
    static FT_Pos prev_rsb_delta = 0;
...
AnimatedMesh* Font2OBJ::getTextMesh(string text, u16 beizerSteps, float extrude, int height, char* filePath, Vector4 color, DEVICE_TYPE device, float bevelRadius, int bevelSegments)
{
    ...
    m = mesh;
    VertexColor = color;
    mesh->indicesType = HIGH_POLY_INDICES;
    ...
    for (int i = 0; i < text.length(); i++) {
        Joint* j = NULL;
        if (text[i] != ' ') {
            j = mesh->addJoint(rootBone);
            boneIndex = j->Index + 1;
        }
        SkinMesh* tempMesh = new SkinMesh();
        tempMesh->indicesType = HIGH_POLY_INDICES;
        ...
        for (int j = 0; j < tempMesh->getIndicesCount(); j++) {
            unsigned int in = tempMesh->getHighPolyIndicesArray()[j];
            mesh->addToHighPolyIndicesArray(in + oldVewrtCount);
        }
        delete tempMesh;
        ...
        if (text[i] != ' ') {
            ...
            j->LocalAnimatedMatrix[12] = -offset - diff / 2.0f;
            j->LocalAnimatedMatrix[13] = height / 2;
            j->LocalAnimatedMatrix[14] = -extrude / 2;
        }
    ...
    if (mesh->getVerticesCount() <= 0 || mesh->getIndicesCount() <= 0)
        return NULL;
