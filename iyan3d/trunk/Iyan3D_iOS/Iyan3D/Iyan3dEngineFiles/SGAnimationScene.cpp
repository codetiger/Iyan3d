...
void SGAnimationScene::saveThumbnail(char* targetPath)
{
    ...
    smgr->Render();
    if(shaderMGR->deviceType == OPENGLES2)
        smgr->writeImageToFile(thumbnailTexture, targetPath , FLIP_VERTICAL);
    ...
    thumbNailImagePath = string(targetPath);
    if(shaderMGR->deviceType == METAL){
        sleep(3.0); // Bad Temporary fix
        smgr->writeImageToFile(thumbnailTexture,(char*)thumbNailImagePath.c_str(),FLIP_VERTICAL);
    }
}
...
//    }
    ...
    if(shaderMGR->deviceType == OPENGLES2)
        smgr->writeImageToFile(renderingTextureMap[RESOLUTION[cameraResolutionType][0]],imagePath,FLIP_VERTICAL);
...
//    }
    ...
    if(shaderMGR->deviceType == METAL){
        smgr->writeImageToFile(renderingTextureMap[RESOLUTION[cameraResolutionType][0]],imagePath);
        smgr->writeImageToFile(renderingTextureMap[RESOLUTION[cameraResolutionType][0]],imagePath);
    }
}
...
void SGAnimationScene::unselectObject(int objectId)
{
    ...
    if(objectId >= 0 && objectId < nodes.size())
    {
        ...
        nodes[objectId]->node->setMaterial(smgr->getMaterialByName(nodes[objectId]->props.prevMatName));
        setJointSpheresVisibility(false);
        selectedJoint = NULL;
        selectedNode = NULL;
        selectedJointId = NOT_EXISTS;
...
void SGAnimationScene::resetMaterialTypes(bool isToonShader)
{
    ...
    for(int index = 0; index < nodes.size(); index++)
    {
        switch (nodes[index]->getType()) {
            case NODE_SGM:
            case NODE_OBJ: {
                nodes[index]->node->setMaterial(smgr->getMaterialByIndex((isToonShader) ? SHADER_TOON : commonType));
                break;
            }
            case NODE_RIG: {
...
void SGAnimationScene::generateSGFDFiles(int startFrame , int endFrame)
{
    if(startFrame == endFrame) {
        SGCloudRenderingHelper::writeFrameData(this, startFrame);
        return;
    }
    
    for(int index = startFrame; index <= endFrame; index++) {
        SGCloudRenderingHelper::writeFrameData(this, index);
    }
