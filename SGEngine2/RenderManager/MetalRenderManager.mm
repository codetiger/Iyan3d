...
void MetalRenderManager::setupMetalLayer() {
    _metalLayer                 = (CAMetalLayer*)renderView.layer;
...
void MetalRenderManager::setupRenderPassDescriptorForTexture(id<MTLTexture> texture, Vector4 color, bool isDepthPass) {
    ...
    if (!isDepthPass) {
        if (ANTI_ALIASING_SAMPLE_COUNT > 1) {
            colorTextureType                                         = MTLTextureType2DMultisample;
            sampleCount                                              = 2;
...
void MetalRenderManager::draw2DImage(Texture* texture, Vector2 originCoord, Vector2 endCoord, Material* material, bool isRTT) {
    setUpDepthState(CompareFunctionLessEqual, false, false);
...
void MetalRenderManager::bindDynamicUniform(Material* material, string name, void* values, DATA_TYPE type, ushort count, u16 paramIndex, int nodeIndex, Texture* tex, bool isFragmentData, bool blurTex) {
    if (tex)
        BindUniform(type, 0, values, count, paramIndex, ((MTLTexture*)tex)->texture, name, NOT_EXISTS, isFragmentData, blurTex);
    else {
        int dataTypeSize = (type == DATA_INTEGER) ? sizeof(int) : sizeof(float);
        int bufIndex     = getAvailableBfferWithSize(count * dataTypeSize);

        uint8_t* bufferPointer = (uint8_t*)[MTLBuffersMap[count * dataTypeSize][bufIndex].buf contents];
        memcpy(bufferPointer, values, count * sizeof(float));

        BindUniform(type, MTLBuffersMap[count * dataTypeSize][bufIndex].buf, values, count, paramIndex, NULL, name, NOT_EXISTS, isFragmentData);
    }
}
...
Texture* MetalRenderManager::createRenderTargetTexture(string textureName, TEXTURE_DATA_FORMAT format, TEXTURE_DATA_TYPE texelType, int width, int height, int storageMode) {
    ...
    texDesc.storageMode           = storageMode;
...
id<MTLTexture> MetalRenderManager::getMSATexture(int sampleCount, int width, int height) {
    ...
    msaaTexDesc.storageMode           = MTLStorageModePrivate;
...
void MetalRenderManager::setRenderTarget(Texture* renderTexture, bool clearBackBuffer, bool clearZBuffer, bool isDepthPass, Vector4 color) {
    ...
    if (renderTexture) {
        ...
        if (!isDepthPass) {
            if (ANTI_ALIASING_SAMPLE_COUNT > 1) {
                sampleCount                                              = ANTI_ALIASING_SAMPLE_COUNT;
                colorTextureType                                         = MTLTextureType2DMultisample;
        ...
        if (_renderPassDescriptor && CMDBuffer) {
            ...
            [rttRenderCMDBuffer setDepthStencilState:_generalDepthWriteEnableState];
